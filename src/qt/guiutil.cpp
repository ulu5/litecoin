...
=======
    widget->setPlaceholderText(QObject::tr("Enter a Ulucoin address (e.g. %1)").arg(
>>>>>>> dev
...
=======
    if(!uri.isValid() || uri.scheme() != QString("ulucoin"))
>>>>>>> dev
        return false;
...
=======
    if(uri.startsWith("ulucoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "ulucoin:");
>>>>>>> dev
    }
...
=======
    QString ret = QString("ulucoin:%1").arg(info.address);
>>>>>>> dev
    int paramCount = 0;
...
=======
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Ulucoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Ulucoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Ulucoin (%s).lnk", chain);
>>>>>>> dev
}
...
=======
        return GetAutostartDir() / "ulucoin.desktop";
    return GetAutostartDir() / strprintf("ulucoin-%s.lnk", chain);
>>>>>>> dev
}
...
=======
            optionFile << "Name=Ulucoin\n";
        else
            optionFile << strprintf("Name=Ulucoin (%s)\n", chain);
>>>>>>> dev
