...
=======
This guide describes how to build ulucoind and command-line utilities on OpenBSD.
>>>>>>> dev
...
=======
The default C++ compiler that comes with OpenBSD 5.9 is g++ 4.2. This version is old (from 2007), and is not able to compile the current version of Ulucoin Core, primarily as it has no C++11 support, but even before there were issues. So here we will be installing a newer compiler.
>>>>>>> dev
...
=======
    test_ulucoin:/usr/lib/libstdc++.so.57.0: /usr/local/lib/libestdc++.so.17.0 : WARNING: symbol(_ZN11__gnu_debug17_S_debug_me ssagesE) size mismatch, relink your program
    ...
    Segmentation fault (core dumped)

This makes it necessary to build boost, or at least the parts used by Ulucoin Core, manually:

```
# Pick some path to install boost to, here we create a directory within the ulucoin directory
>>>>>>> dev
LITECOIN_ROOT=$(pwd)
...
=======
# Build w/ minimum configuration necessary for ulucoin
>>>>>>> dev
