...
=======
Ulucoin Core version 0.13.2 is now available from:

  <https://download.ulucoin.org/ulucoin-0.13.2.1/>
>>>>>>> dev
...
=======
  <https://github.com/ulucoin-project/ulucoin/issues>
>>>>>>> dev
...
=======
updates will be released anymore. Without security updates, using a ulucoin
>>>>>>> dev
...
=======
ECDSA signatures inside Ulucoin transactions now use validation using
>>>>>>> dev
...
=======
Previous versions of Ulucoin Core had their mempool limited by checking
>>>>>>> dev
...
=======
for previous versions of Ulucoin Core was to raise the default minimum
relay fee.

Ulucoin Core 0.13.2 will have a strict maximum size on the mempool. The
>>>>>>> dev
...
=======
Ulucoin Core 0.13.2 also introduces new default policy limits on the length and
>>>>>>> dev
...
=======
This allows running ulucoind without having to do any manual configuration.
>>>>>>> dev
...
=======
Ulucoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Ulucoin Core will relay transactions with
>>>>>>> dev
...
=======
entirely in Ulucoin Core 0.13, and it is at this time undecided whether the
>>>>>>> dev
...
=======
Ulucoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Ulucoin Core automatically creates a hidden service to listen on, without
manual configuration. Ulucoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Ulucoin Core is listening, and
>>>>>>> dev
...
=======
Ulucoind can now (optionally) asynchronously notify clients through a
>>>>>>> dev
...
=======
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Ulucoin
Core to use floating fees. By default, Ulucoin Core will use floating
>>>>>>> dev
fees.
...
=======
(default: `0.0002` ULU/kB).

At all times, Ulucoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) ULU.
Furthermore, Ulucoin Core will never create transactions paying less than
>>>>>>> dev
...
=======
`ulucoin.conf`, where `N` is the number of MiB to allot for
>>>>>>> dev
raw block & undo data.
...
=======
        # optional bypass ulucoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< ulucoinrpc:<password>
>>>>>>> dev
    </Location>
...
=======
Restarting ulucoind will no longer clear out the list of banned peers; instead
>>>>>>> dev
...
=======
- Adding `dbcache=100` in ulucoin.conf
>>>>>>> dev
...
=======
    $ src/ulucoin-cli -stdin walletpassphrase
>>>>>>> dev
    mysecretcode
    120
...
=======
Various code modernizations have been done. The Ulucoin Core code base has
>>>>>>> dev
...
=======
- `ulucoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `ulucoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
>>>>>>> dev
...
=======
for a new opcode in the Ulucoin scripting system that in combination with
>>>>>>> dev
...
=======
Ulucoin transactions currently may specify a locktime indicating when
>>>>>>> dev
...
=======
HD wallets are incompatible with older versions of Ulucoin Core.
>>>>>>> dev
...
=======
The two phases are distinct in the Ulucoin-Qt GUI. During the first one,
>>>>>>> dev
...
=======
  malleability, which is a problem that makes programming Ulucoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Ulucoin.
>>>>>>> dev
...
=======
  each Ulucoin block need to be stored by nodes in order to validate future
>>>>>>> dev
...
=======
  of ulucoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of ulucoins being spent is exactly
>>>>>>> dev
...
=======
- **Increased security for multisig:** Ulucoin addresses (both P2PKH addresses
>>>>>>> dev
...
=======
  believed to be provided by Ulucoin's choice of parameters for its Elliptic
>>>>>>> dev
...
=======
  produce an accurate copy of Ulucoin's current ledger (called the UTXO set),
>>>>>>> dev
...
=======
  note that Ulucoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Ulucoin users to individually opt-in to almost any change in the Ulucoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Ulucoin Core contributors that may
>>>>>>> dev
...
=======
segwit's start date of 1 January 2017 miners can update the Ulucoin
client to Ulucoin Core 0.13.2 to signal for segwit support. When a
>>>>>>> dev
...
=======
Since Ulucoin Core 0.10.0, nodes have defaulted to only relaying and
>>>>>>> dev
...
=======
- `ulucoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `ulucoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
>>>>>>> dev
